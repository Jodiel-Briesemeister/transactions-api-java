...
public class DepositUseCase {
    ...
    public DepositUseCase(AccountRepository accountRepository, TransactionRepository transactionRepository,
    ...
    public void execute(String userId, long amount) {
        ...
        // No lock needed: a credit never has to read the balance first, and updateBalance is a
        // single atomic UPDATE ... SET balance = balance + :delta.
        ...
        Transaction transaction = Transaction.create(userId, TransactionType.DEPOSIT, amount, null);
        transactionRepository.create(transaction);
        accountRepository.updateBalance(userId, amount);

        log.info("Deposit completed userId={} amount={}", userId, amount);

        User user = userRepository.findById(userId)
                .orElseThrow(() -> new AppException("User not found", 404));

        messagePublisher.publish(Queue.NOTIFICATIONS_EMAIL, Map.of(
                "templateId", NotificationTemplate.TRANSACTION_DEPOSIT.getValue(),
